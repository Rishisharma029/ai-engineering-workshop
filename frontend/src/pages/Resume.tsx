...
export default function Resume({ activeProject }: ResumeProps) {
  ...
  // Recruiter Pitch
  const recruiterPitch = `Hi there,\n\nI just finished designing an AI Engineering Workshop platform that automates codebase audits. The application features a dynamic static analyzer checking for CVSS vulnerabilities, a local RAG engine answering architectural questions, and zoomable SVG force topologies drawing dependency maps.\n\nI'd love to chat about how my experience in TypeScript, Node.js, and AI engineering fits your current software development projects. Let me know if you have time for a quick prescreen!\n\nBest,\n${fullName}`;
  ...
  const portfolioDesc = `Built an AI Engineering Workshop platform auditing codebase files. Features interactive SVG topological dependency trees, CVSS threat radar checklist controls, and mock interview prep viva simulators.`;
  ...
  const handleDownloadResumeMarkdown = () => {
    const markdown = `# ${fullName}\n\n` +
      ...
      `### ${activeProject.name} (AI Engineering Workshop)\n` +
      `*${portfolioDesc}*\n\n` +
      `**Key Accomplishments:**\n` +
      ...
      `---\n*Generated by AI Engineering Workshop V6*\n`;
  ...
  const handleDownloadResumeText = () => {
    const text = `====================================================\n` +
      ...
      `PROFILE SUMMARY:\n` +
      `${linkedinSummary}\n\n` +
      `PROJECT: ${activeProject.name} (AI Engineering Workshop)\n` +
              ...
              <div className="space-y-2.5 select-text">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <span className="text-[10px] text-slate-500 font-semibold uppercase font-mono">AI Engineering Workshop</span>
                  </div>
